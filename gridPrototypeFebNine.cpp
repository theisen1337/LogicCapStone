        ...
        for (int j = 0; j < tiles[i].size(); j++)
        {
            if (tiles[i][j][0] == "L") //for expanding the Lakes
            {
                ...
                else
                {
                    ...
                    else if (randy == 0 && i == tiles.size()-1)
                    {
                        cout << randy; //DEBUG
                        cout << "botEdge";
                        if (j == tiles.size()-1)
...
vector < vector < vector<string> > > setOres(string oreLetter[], vector < vector < vector<string> > > &ores, int aLen)
{
    //cout << aLen << endl; //DEBUG
...
int main()
{
    ...
    */
    ...
    int tilLen = sizeof(tile)/sizeof(tile[0]); //will be changed. the function can get this variable inside of the function
